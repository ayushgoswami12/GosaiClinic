...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error("[v0] Error fetching patient from JSONBin:", error)
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const patientExists = patients.some((p: any) => p.id === params.id)
    if (!patientExists) {
    ...
    console.error("[v0] Error updating patient in JSONBin:", error)
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error("[v0] Error deleting patient from JSONBin:", error)
